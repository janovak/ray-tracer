...
__global__ void RenderScene(Color* image, unsigned int width, unsigned int height, unsigned int samples_per_pixel, Hittable** world, curandState* rand_state) {
    ...
    if (idx < width && idy < height) {
        ...
        image[pixel_index] = Color::LinearToGamma(color / static_cast<float>(samples_per_pixel));
    }
}
...
int RayTracer::WriteToFile(std::string filename) {
...
__device__ Color RayTracer::RayColor(const Ray& ray, Hittable** world, curandState* rand_state) {
    Ray current_ray = ray;
